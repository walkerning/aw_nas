...
class DenseRobFinalModel(FinalModel):
    ...
    def __init__(self, search_space, device, genotypes,
        ...
        for i_layer, stride in enumerate(strides):
            ...
            cell = DenseRobCell(self.search_space,
    ...
    def _is_reduce(self, i_layer):
        return self.search_space.cell_layout[i_layer] in self.search_space.reduce_cell_groups


class DenseRobCell(nn.Module):
    def __init__(self, search_space, cell_arch, num_input_channels, num_out_channels, stride, prev_strides):
    ...
    def forward(self, inputs, dropout_path_rate): #pylint: disable=arguments-differ
        ...
        for to_ in range(self.num_init_nodes, self._num_nodes):
            state_to_ = torch.zeros(
                [batch_size, self.num_out_channels, o_height, o_width], device=states[0].device)
