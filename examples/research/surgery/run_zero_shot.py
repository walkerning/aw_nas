...
for data_shuffle_file in data_shuffle_indices_files:
    ...
    if data_shuffle_file is not None:
        ...
        can_ds_fname = os.path.basename(data_shuffle_file).split(".", 1)[0].replace("-", "_")
    else:
        can_ds_fname = "ori"
...
if args.evaluators:
    evaluator_paths = args.evaluators.split(",")
...
print("#Cfgs: {}; #Processes: {}; #GPUs: {}".format(len(all_cfgs), num_processes, len(np.unique(GPUs))))
...
def _worker(p_id, gpu_id, queue):
    while 1:
        ...
        if isinstance(init_seed_or_eval_path, str):
            seed = 20
            load_str = "--load {}/evaluator --seed {}".format(init_seed_or_eval_path, seed)
        ...
        cmd = ("awnas eval-arch {} {} {} --steps {} --gpu {} --reset-dataloader-each-rollout"
               " --dump-rollouts {} >{} 2>&1")\
               .format(
                   eval_cfg_fname, args.arch_file, load_str, n_batch, gpu_id, out_file, eval_log)
        print("Process #{}: GPU {} ({}); CMD: {}".format(
            p_id, gpu_id, cfg, cmd))
