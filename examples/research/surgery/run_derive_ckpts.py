...
parser.add_argument("--cfg-ckpt-rel-path", default=None,
                    help="The relative path of cfg file to ckpt dir")
parser.add_argument("--arch-file", default=None)
parser.add_argument("--arch-file-ckpt-rel-path", default=None,
                    help="The relative path of arch file to ckpt dir")
parser.add_argument("--result-dir", required=True)
parser.add_argument("--subset", default=False, action="store_true",
                    help="Only applicable when using derive (nb201)")
parser.add_argument("--addi-controller-cfgs", default="",
                    help="Add controller cfgs by yaml dict, "
                    "Only relevant when using derive (nb201)")
...
assert args.cfg_file is not None or args.cfg_ckpt_rel_path is not None, \
...
if not args.cfg_ckpt_rel_path:
    ...
    if eval_or_derive_cfg["search_space_type"] == "nasbench-201":
        ...
        if args.subset:
            ...
            if not args.iso:
                derive_cfg["controller_cfg"]["text_file"] = \
                        os.path.join(_HOME_DIR, "awnas/data/nasbench-201/non-isom.txt")
                arch_num = 6466
            else:
                derive_cfg["controller_cfg"]["text_file"] = \
                        os.path.join(_HOME_DIR, "awnas/data/nasbench-201/iso.txt")
                arch_num = 15625
    else:
    ...
    for ckpt in ckpts:
        ...
        if eval_or_derive_cfg["search_space_type"] == "nasbench-201":
            ...
            if args.addi_controller_cfgs:
                derive_cfg = copy.deepcopy(eval_or_derive_cfg)
                derive_cfg["controller_cfg"].update(yaml.load(StringIO(args.addi_controller_cfgs)))
...
res_files = [os.path.relpath(ckpt, common_path).strip("/") + (".yaml" if call_derive else ".pkl")
             for ckpt, call_derive in zip(ckpts, call_derives)]

os.makedirs(args.result_dir, exist_ok=True)
for res_file in res_files:
    if "/" in res_file:
        os.makedirs(os.path.join(args.result_dir, res_file.rsplit("/", 1)[0]), exist_ok=True)
    ...
    while 1:
        ...
        if call_derive:
            ...
            cmd = ("awnas derive {} --load {} --out-file {} --gpu {} -n {} --test --seed 123 "
                   "--runtime-save >{} 2>&1").format(cfg_file, ckpt_dir, out_file, gpu_id,
                                                     num_arch, derive_log)
        else:
            # call eval-arch
            cmd = ("awnas eval-arch {} {} --load {} --dump-rollouts {} --gpu {} --seed 123 "
                   ">{} 2>&1").format(cfg_file, arch_file, ckpt_dir, out_file, gpu_id, derive_log)
