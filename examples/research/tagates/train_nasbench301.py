...
def train(train_loader: DataLoader, model: ArchNetwork, epoch: int, args: argparse.Namespace) -> float:
    ...
    for step, (archs, zs_p, f_accs) in enumerate(train_loader):
        ...
        if args.compare:
            ...
            zs_p_1, zs_p_2 = zs_p[ex_thresh_inds[1]], zs_p[ex_thresh_inds[0]]
            archs_1 = (archs_1, zs_p_1)
            archs_2 = (archs_2, zs_p_2)
...
def main(argv):
    parser = argparse.ArgumentParser(prog = "train_nasbench301.py")
    ...
    train_data = [[d[0], d[1], d[2]] for d in train_data]
    valid_data = [[d[0], d[1], d[2]] for d in valid_data]
