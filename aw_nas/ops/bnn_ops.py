...
class xnor_binarize(torch.autograd.Function):
    @staticmethod
    def forward(self, x, fp_weight, fp32_act):
        ...
        self.save_for_backward(old_x, fp_weight, bi_weight, mean_val)
        return x, bi_weight

    @staticmethod
    def backward(self, g_x, g_bi_weight):
        x, fp_weight, bi_weight, mean_val = self.saved_tensors
        g_x[x.ge(1)] = 0
        g_x[x.le(-1)] = 0

        proxy = fp_weight.abs().sign()
        proxy[fp_weight.abs()>1] = 0
        ...
        g_fp_weight = g_fp_weight * fp_weight[0].nelement() * (1-1/fp_weight.size(1))

        return g_x, g_fp_weight, None

class XNORConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups, dropout_ratio=0, fp32_act=False):
        ...
        self.full_precision = nn.Parameter(torch.zeros([self.out_channels, self.in_channels, self.kernel_size, self.kernel_size]))
    ...
    def forward(self, x):
        ...
        x = F.conv2d(x, bi_weight, bias=self.bias, stride=self.stride, padding=self.padding)
        return x

class XNORConvBNReLU(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding,affine=True,groups=1, dropout_ratio=0, fp32_act=False):
...
class BinaryResNetBlock(nn.Module):
    def __init__(self, C, C_out, stride, affine, kernel_size=3, block="bireal",act=BinaryActivation,downsample="conv", fp32_act=False):
