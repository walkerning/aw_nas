...
def ofa_rollout_to_primitive(rollout, spatial_size, primitive_type, strides, channels, activation=None, use_se=None):
    primitives = []
    if activation is None:
        activation = [None] * len(rollout.depth)
    if use_se is None:
        use_se = [None] * len(rollout.depth)
    sizes = [round(spatial_size / reduce(lambda p, q: p * q, strides[:i])) for i in range(1, len(strides) + 1)]
    for i, (depth, size, s, c_in, c_out, act, se) in enumerate(
        ...
        for j, width, kernel in zip(
            ...
            if i > 0:
                c_in = c_out
                s = 1
            primitives.append(Prim(primitive_type, size, c_in, c_out, s, kernel, activation=act, use_se=se, expansion=width))
    return primitives

class OFAHardwareObjectiveModel(BaseHardwareObjectiveModel):
    NAME = "ofa"

    def __init__(self, prof_prims, prof_prims_cfg, base_channels, mult_ratio, strides, activation=None, use_se=None, spatial_size=112, schedule_cfg=None):
        super(OFAHardwareObjectiveModel, self).__init__(schedule_cfg)
        self.default_prim_type = prof_prims_cfg.get("default_prim_type", "mobilenet_v2_block")
    ...
    def _orgnize_table(self):
        ...
        for prim in self.prof_prims:
            perf = self.Perf(*[prim.pop(f) if f in prim else 0.  for f in self.performances])
            prim = Prim(**prim)
            self._table[prim] = perf


    def predict(self, rollout):
        primtives = ofa_rollout_to_primitive(rollout, self.default_prim_type, self.spatial_size, self.strides, self.channels, self.activation, self.use_se)
        perfs = []
        for prim in primtives:
            perf = self._table[prim.prim_type].get(prim)
            if perf is None:
                logger.warn(f"primitive {prim} is not found in the table, return default value 0.")
                perf = self.Perf(*[0. for f in self.performances])
            perfs.append(perf)
...
class OFAMixinProfilingSearchSpace(MNasNetOFASearchSpace, MixinProfilingSearchSpace):
    ...
    def __init__(self, width_choice, depth_choice, kernel_choice, num_cell_groups, expansions, fixed_primitives=None, schedule_cfg=None):
        super(OFAMixinProfilingSearchSpace, self).__init__(width_choice, depth_choice, kernel_choice, num_cell_groups, expansions, schedule_cfg=schedule_cfg)
    ...
    def generate_profiling_primitives(self, base_channels, mult_ratio, strides, activation=None, use_se=None, primitive_type="mobilenet_v2_block", spatial_size=112, sample=None, as_dict=True):
