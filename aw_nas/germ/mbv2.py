...
def schedule_choice_callback(choices: germ.Choices, epoch: int, schedule: List[dict]) -> None:
    """
    Args:
    ...
    print("Epoch: {:>4d}, decision id: {}, choices: {}".format(epoch, choices.decision_id,
        choices.choices))
...
class MobileNetV2(germ.SearchableBlock):
    ...
    def __init__(self,
        ...
        kernel_choices_cb = functools.partial(schedule_choice_callback,
                schedule=schedule_cfg.get("kernel_sizes")) 
        exp_choices_cb = functools.partial(schedule_choice_callback,
                schedule=schedule_cfg.get("expansion_choices"))
        width_choices_cb = functools.partial(schedule_choice_callback,
                schedule=schedule_cfg.get("mult_ratio_choices"))
        depth_choices_cb = functools.partial(schedule_choice_callback,
                schedule=schedule_cfg.get("depth_choices"))

        prev_channels = make_divisible(self.channels[1] * max(self.mult_ratio_choices), 8)
        ...
        for i, stride in enumerate(self.strides):
            stage = nn.ModuleList([])
            cur_channels = germ.Choices(mult_ratio_choices, epoch_callback=width_choices_cb,
                    post_mul_fn=divisor_fn) * self.channels[i + 2]
            ...
            if i < len(self.strides) - 1:
                self.depth_decs[str(i)] = germ.Choices(depth_choices, epoch_callback=depth_choices_cb)
            for j in range(max(depth_choices)):
                exp_ratio = germ.Choices(expansion_choices, epoch_callback=exp_choices_cb,
                        post_mul_fn=divisor_fn)
                kernel_choice = germ.Choices(self.kernel_sizes, epoch_callback=kernel_choices_cb) 
        ...
        if pretrained_path:
            ...
            if "classifier.weight" in state_dict and state_dict["classifier.weight"].shape[0] != self.num_classes:
        ...
        with self.finalize_context(rollout):
            ...
            for i, cell in enumerate(self.cells):
                if str(i) in self.depth_decs:
                    ...
                    depth = len(cell)
                cells.append(nn.ModuleList([c.finalize_rollout(rollout) for c in cell[:depth]]))
            self.cells = cells
...
class MBV2SuperNet(germ.GermSuperNet):
    ...
    def get_feature_channel_num(self, p_levels):
        ...
        return [self.backbone.cells[level_indexes[p]][-1].out_channels for p in p_levels]
