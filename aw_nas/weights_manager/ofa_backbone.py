...
from aw_nas.utils.common_utils import nullcontext, _get_channel_mask
...
class FlexibleMobileNetV2Block(MobileNetV2Block, FlexibleBlock):
    ...
    def __init__(self, expansion, C, C_out, 
                 stride, 
                 kernel_sizes=(3, 5, 7), do_kernel_transform=True, 
                 activation='relu',
                 affine=True,
                 schedule_cfg=None):
        ...
        depth_wise = nn.Sequential(
            FlexibleDepthWiseConv(C_inner, self.kernel_sizes, stride, do_kernel_transform=do_kernel_transform),
            FlexibleBatchNorm2d(C_inner, affine=affine),
            act_fn(inplace=True)
        )
        ...
        super(FlexibleMobileNetV2Block, self).__init__(expansion, C, C_out, stride, self.kernel_size, activation, inv_bottleneck, depth_wise, point_linear)
    ...
    def finalize(self):
        inv_bottleneck = None
        if self.inv_bottleneck:
            inv_bottleneck = nn.Sequantial(
                **[m.finalize() if isinstance(m, FlexibleLayer) else m for m in self.inv_bottleneck]
            )
        
        depth_wise = nn.Sequantial(
                **[m.finalize() if isinstance(m, FlexibleLayer) else m for m in self.depth_wise]
            )
        point_linear = nn.Sequantial(
                **[m.finalize() if isinstance(m, FlexibleLayer) else m for m in self.point_linear]
            )
        return MobileNetV2Block(self.expansion, self.C, self.C_out, self.stride, self.kernel_size, self.activation, inv_bottleneck, depth_wise, point_linear)
...
class FlexibleMobileNetV3Block(MobileNetV3Block, FlexibleBlock):
    ...
    def __init__(self, expansion, C, C_out, 
        ...
        depth_wise = nn.Sequential(
            FlexibleDepthWiseConv(self.C_inner, self.kernel_sizes, stride, do_kernel_transform=do_kernel_transform),
            FlexibleBatchNorm2d(self.C_inner, affine=affine),
            self.act_fn(inplace=True)
        )
        ...
        super(FlexibleMobileNetV3Block, self).__init__(expansion, C, C_out, stride, self.kernel_size, activation, use_se, inv_bottleneck, depth_wise, point_linear, se)
        self.reset_mask()
    ...
    def finalize(self):
        ...
        se = self.se.finalize()
        return MobileNetV3Block(self.expansion, self.C, self.C_out, self.stride, self.kernel_size, self.activation, self.use_se, inv_bottleneck, depth_wise, point_linear, se)
...
class BaseBackboneArch(Component, nn.Module):
    ...
    @abc.abstractmethod
    def make_stage(self, C_in, C_out, depth, stride, expansion, kernel_size, mult_ratio=1.):
        """
...
class MobileNetV2Arch(BaseBackboneArch):
    ...
    def __init__(self, device, 
                 ...
                 schedule_cfg=None):
        super(MobileNetV2Arch, self).__init__(device, blocks, strides, 
                                              expansions, channels, mult_ratio, 
                                              kernel_sizes, do_kernel_transform, 
                                              num_classes, schedule_cfg)
        ...
        expect(blocks[0] == expansions[0] == 1, "The first conv layer should have single block and no expansion.", ValueError)
        self.mult_ratio = mult_ratio

        self.cells = [
            self.make_stage(
                    self.channels[0], self.channels[1],
                    self.blocks[0], self.strides[0],
                    self.expansions[0], [3]
                )
        ]
        for i, depth in enumerate(self.blocks[1:], 1):
            self.cells.append(
                self.make_stage(
                    ...
                    depth, self.strides[i],
                    self.expansions[i], self.kernel_sizes
                )
            )
    ...
    def make_stage(self, C_in, C_out, block_num, stride, expansion, kernel_sizes):
        ...
        for i in range(block_num):
            if i == 0:
                ...
                C_in = C_out
            cell.append(
                self.block_initializer(expansion, C_in, C_out, s,
                                       kernel_sizes, self.do_kernel_transform, 'relu', affine=True))
        return nn.ModuleList(cell)

    def forward(self, inputs):
        return self.forwrd_rollout(inputs)
        ...
        for i, cell in enumerate(self.cells):
            for j, block in enumerate(cell):
                if rollout is None:
                    ...
                    try:
                        out = block.forward_rollout(out, rollout.width[i][j], rollout.kernel[i][j])
                    except:
...
class MobileNetV3Arch(BaseBackboneArch):
    ...
    def __init__(self, device, 
                 ...
                 schedule_cfg=None):
        super(MobileNetV3Arch, self).__init__(device, blocks, strides, 
                                              expansions, channels, mult_ratio, 
                                              kernel_sizes, do_kernel_transform, 
                                              num_classes, schedule_cfg)
        self.block_initializer = FlexibleBlock.get_class_(block_type)
        ...
        self.cells = [
            self.make_stage(
                    self.channels[0], self.channels[1],
                    self.blocks[0], self.strides[0],
                    self.expansions[0], [3], self.use_ses[0], self.acts[0]
                )
        ]
        for i, depth in enumerate(self.blocks[1:], 1):
            self.cells.append(
                self.make_stage(
                    self.channels[i], self.channels[i + 1],
                    depth, self.strides[i],
                    self.expansions[i], self.kernel_sizes, self.use_ses[i], self.acts[i]
                )
            )
        self.cells = nn.ModuleList(self.cells)
        self.conv_head = nn.Sequential(
    ...
    def make_stage(self, C_in, C_out, block_num, stride, expansion, kernel_sizes, use_se, act):
        cell = []
        for i in range(block_num):
            if i == 0:
                s = stride
            ...
            cell.append(
                self.block_initializer(expansion, C_in, C_out, s,
                                       kernel_sizes, self.do_kernel_transform, act, affine=True, use_se=use_se))
        return nn.ModuleList(cell)
