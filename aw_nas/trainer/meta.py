...
class MetaTrainer(BaseTrainer):
    ...
    def __init__(self, #pylint: disable=dangerous-default-value
        ...
        Args:
            ...
            meta_update_method (str): for meta update: `method` refers to
                ...
                reptile: REPTILE
        """
        super(MetaTrainer, self).__init__(controller, evaluator, rollout_type, schedule_cfg)
        ...
        if self.derive_steps is None:
            ...
            if suggested is not None and not suggested == self.derive_steps:
                self.logger.warning("The suggested `derive_steps` (%3d) from "
                                    "`evaluator.suggested_derive_steps_per_epoch()` differs "
                                    "from the config setting (%3d).",
                                    suggested, self.derive_steps)
        ...
        if isinstance(self.controller, torch.nn.Module):
            ...
            self.controller_base_optimizer = utils.init_optimizer(
                self.controller.base_update_parameters(),
                controller_base_optimizer
            )
            self.controller_base_scheduler = utils.init_scheduler(
                self.controller_base_optimizer,
                controller_base_scheduler
            )
        ...
        if train_base_style not in ["together", "evaluator"]:
            raise Exception(f"Unrecognized base update style `{train_base_style}` for training.")
        if test_base_style not in ["together", "evaluator"]:
            raise Exception(f"Unrecognized base update style `{test_base_style}` for test.")
    ...
    def _together_update(
        ...
        rollouts = self.evaluator.evaluate_rollouts(
            ...
            callback=partial(
                self._backward_rollout_to_controller,
                step_loss=step_loss
            ) if is_training and self.backward_query else None
        )
        self.evaluator.update_rollouts(rollouts)
    ...
    def _together_update_epoch(self, is_training=True):
        ...
        for i_meta in range(1, meta_steps + 1):
            ...
            for i_batch in range(1, meta_batch_size + 1):
                ...
                if rollout_stats is not None:
                    ...
                    query_perf = "; ".join([f"{k}: {v:.3f}" for k, v in rollout_stats.items()])
                    print((f"\rmeta step {i_meta:4d}/{meta_steps:4d}, "
                ...
                if is_training:
                    self.controller.meta_gradient(self.meta_update_method,
                                                  self.controller_base_optimizer.param_groups[0]["lr"]
                                                  if self.controller_base_optimizer is not None else None)
            ...
            if self.interleave_report_every and i_meta % self.interleave_report_every == 0:
                print("\r", end="")
                self.logger.info(
        ...
        print("\r", end="")
        return eva_stat_meters, rollout_stat_meters, c_stat_meters, c_loss_meter, update_time_meter
    ...
    def train(self): #pylint: disable=too-many-branches
        ...
        for epoch in range(self.last_epoch+1, self.epochs+1):
            ...
            if self.train_base_style == "together":
                eva_stat_meters, rollout_stat_meters, c_stat_meters, c_loss_meter, update_time_meter = \
            ...
            if update_time_meter:
                self.logger.info("Epoch %3d [update time] %.3f", epoch, update_time_meter.avg)
    ...
    def meta_test(self, epoch):
        ...
        # log infomations of this epoch
        if eva_stat_meters:
            self.logger.info("META TEST Epoch %3d: [evaluator update] %s", epoch,
                             "; ".join(["{}: {:.3f}".format(n, v) \
                                        for n, v in eva_stat_meters.avgs().items()]))
        if rollout_stat_meters:
            self.logger.info("META TEST Epoch %3d: [controller update] controller loss: %.3f ; "
                             "rollout performance: %s", epoch, c_loss_meter.avg,
                             "; ".join(["{}: {:.3f}".format(n, v) \
                                        for n, v in rollout_stat_meters.avgs().items()]))
        if c_stat_meters:
            self.logger.info("[controller stats] %s", \
                             "; ".join(["{}: {:.3f}".format(n, v) \
                                        for n, v in c_stat_meters.avgs().items()]))
        if update_time_meter:
            self.logger.info("META TEST Epoch %3d [update time] %.3f", epoch, update_time_meter.avg)
    ...
    def save(self, path):
        base_optimizer_state = self.controller_base_optimizer.state_dict()\
            if self.controller_base_optimizer is not None else None
        base_scheduler_state = self.controller_base_scheduler.state_dict()\
            if self.controller_base_scheduler is not None else None
        meta_optimizer_state = self.controller_meta_optimizer.state_dict()\
            if self.controller_meta_optimizer is not None else None
        meta_scheduler_state = self.controller_meta_scheduler.state_dict()\
            if self.controller_meta_scheduler is not None else None
        state_dict = {
            "epoch": self.epoch,
    ...
    def load(self, path):
        ...
        if self.controller_baseoptimizer is not None:
            self.controller_base_optimizer.load_state_dict(checkpoint["controller_base_optimizer"])
        if self.controller_base_scheduler is not None:
            self.controller_base_scheduler.load_state_dict(checkpoint["controller_base_scheduler"])
        if self.controller_meta_optimizer is not None:
            self.controller_meta_optimizer.load_state_dict(checkpoint["controller_meta_optimizer"])
        if self.controller_meta_scheduler is not None:
            self.controller_meta_scheduler.load_state_dict(checkpoint["controller_meta_scheduler"])
