...
import traceback
import sys
sys.path.insert(0, "/data/liwenshuo/LPCVC")
...
def _conv2d(raw,input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    if id(input) not in log._blobs.data.keys():
        return x
...
def _pool(type,raw,input,x,kernel_size,stride,padding,ceil_mode):
    ...
    layer.pool_param(type=type.upper(), kernel_size=kernel_size, stride=kernel_size if stride is None else stride,
                     pad=padding, ceil_mode=ceil_mode)
    log.cnet.add_layer(layer)
...
def _iadd(input, *args):
    ...
    if id(args[0]) not in log._blobs.data.keys() or id(input) not in log._blobs.data.keys():
        return x
    if not NET_INITTED:
        return x 
    x=x.clone()
